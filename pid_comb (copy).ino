...
void distanceCallback(const std_msgs::Float32MultiArray& msg) {
  ...
  for (int i = 0; i < 4; i++) {  // NUMBER OF SENSORS
    ...
    // Check if distance is NaN or -inf, and skip if so
    if (isnan(distance) || isinf(distance) || distance < 0) {
      Serial.print("Sensor ");
      Serial.print(i+1);
      Serial.println(" has invalid data. Skipping...");
  ...
  if (isinf(minDistance)) {
    ...
    // Depending on the state, you might want to stop or handle differently
    // For safety, we'll stop the motor here
    digitalWrite(motorIN1, LOW);
    digitalWrite(motorIN2, LOW);
    analogWrite(motorPWM, 0);
